    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_EVENT_KEY_DOWN) {
                ...
                if (e.key.key == SDLK_V) {
                    rend.scope_view = (HdrplayScopeView)
                        ((rend.scope_view + 1) % HDRPLAY_SCOPE_COUNT);
                    LOG("REND", "scope -> %s",
                ...
                if (n_sources > 1) {
                    if (e.key.key == SDLK_0) {
                        rend.solo = -1; rend.diff_view = false;
                        LOG("REND", "compare A|B");
                    }
                    if (e.key.key == SDLK_1) {
                        rend.solo = 0; rend.diff_view = false;
                        LOG("REND", "solo %s", sources[0].label);
                    }
                    if (e.key.key == SDLK_2) {
                        rend.solo = 1; rend.diff_view = false;
                        LOG("REND", "solo %s", sources[1].label);
                    }
                    if (e.key.key == SDLK_X) {
